package CRItesting.json

import java.io.File

import CRItesting.CRIManifestFile
...
/**
  ...
  */
object CRIFileProcessor extends App {
  ...
  def processLogFile(file:File, hiveContext: HiveContext): Unit = {
    ...
    val txn_event_df1 = txn_event_df.select(col("timestamp").alias("APPLIANCE_TS_UTC"), col("json_signature").alias("EPHEMERAL_HMAC"),
      ...
      withColumn("SOURCE_LOG_ENV", lit(source_log_evn)).withColumn("CLUSTER_FOLDER_NAME", lit(cluster_folder_name)).withColumn("BATCH_ID", lit(batch_id)).withColumn("CLUSTER_ID", lit(cluster_id)).withColumn("TXN_LOG_SEQNUM", lit(txn_seqnum))

    txn_event_df1.printSchema()
    txn_event_df1.show()

    txn_event_df1.write.mode("append").saveAsTable("cri_transaction_event_log")
